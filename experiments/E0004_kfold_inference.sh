#!/bin/bash
experiment_name="E0004_kfold_inference"
criterion_=( "MSELoss" )
train_dataset_=( "None" )
train_dataset_args_=( "[]" )
train_dataset_kwargs_=( '{}' )
test_dataset_=( "cvvalid" )
test_dataset_args_=( "[]" )
test_dataset_kwargs_=( '{"fold":0}' '{"fold":1}' '{"fold":2}' '{"fold":3}' '{"fold":4}' '{"fold":5}' '{"fold":6}' '{"fold":7}' '{"fold":8}' '{"fold":9}')
model_=( "bilstm" )
model_args_=( "[448,1378]" )
model_kwargs_=( "{}" )
inference_model_=( "logs/E0001_first_model/run_0/checkpoints/model.best.pth" )
optimizer_=( "Adam" )
optim_kwargs_=( "{}" )
batch_size_=( "64" )
lr_=( "1e-6" )
...
inference_model="logs/E0001_first_model/run_"$run"/checkpoints/model.best.pth"
